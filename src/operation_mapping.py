...
def _all_details(d: Detail) -> bool:
    """
    Все детали заказа. Для контроля комплектности на упаковке: через операцию
    проходит каждая деталь, план = весь состав заказа. «Выполнено» = собраны
    все детали (недостача видна как незакрытая операция).
    """
    return True
...
OPERATION_RULES: list[OperationRule] = [
    ...
    OperationRule(
        ...
        stage="packing",
        detail_predicate=_all_details,
        note="ПОДЕТАЛЬНАЯ: контроль комплектности при упаковке — сканируется "
             "каждая деталь заказа, план = весь состав, недостача = незакрытая",
    ),
    OperationRule(
